...
interface Props {
  ...
  onShowHelp?: (type: LogicGateType) => void
}

export default function GatePalette({ onDragStart, onShowHelp }: Props) {
      ...
      <div className="flex flex-col gap-1.5">
        {PALETTE_ITEMS.map(({ type }) => (
          ...
          >
            ...
            <span className="text-xs text-gray-700 dark:text-gray-300 leading-tight flex-1 truncate">
              {uk.logic.gates[type]}
            </span>
            {onShowHelp && (
              <button
                ...
                draggable={false}
                className="text-gray-400 hover:text-accent-500 dark:text-gray-500 dark:hover:text-accent-400 flex-shrink-0"
                aria-label={`${uk.logic.helpPanel}: ${uk.logic.gates[type]}`}
                title={uk.logic.helpPanel}
              >
