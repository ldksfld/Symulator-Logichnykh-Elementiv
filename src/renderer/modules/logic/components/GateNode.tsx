...
export default memo(function GateNode({ id, data, selected }: NodeProps) {
  ...
  const { gateType, label, inputCount, constantValue, inputValue } = d
  ...
  const handleToggleConstant = () => {
    if (gateType !== 'CONSTANT' && gateType !== 'INPUT') return
  ...
  return (
    <div
      className={clsx(
        'relative select-none rounded border-2 bg-white dark:bg-gray-800 shadow-sm min-w-[70px] cursor-default',
        selected
      ...
      onKeyDown={isToggleable ? (e) => { if (e.key === 'Enter' || e.key === ' ') handleToggleConstant() } : undefined}
    >
