...
function tokenize(expr: string): Token[] {
  ...
  while (i < e.length) {
    ...
    if (ch === '!' || ch === '~' || ch === '¬') {
      tokens.push({ type: 'NOT', value: ch }); i++; continue
    }
    if (ch === '&' || ch === '·' || ch === '∧') {
      if (e[i+1] === '&') i++
      tokens.push({ type: 'AND', value: ch }); i++; continue
    }
    if (ch === '|' || ch === '+' || ch === '∨') {
      if (e[i+1] === '|') i++
      tokens.push({ type: 'OR', value: ch }); i++; continue
    }
    if (ch === '^') { tokens.push({ type: 'XOR', value: ch }); i++; continue }

    const rest = e.slice(i)
    const kw = rest.match(/^(AND|OR|NOT|XOR)\b/i)
    if (kw) {
      const k = kw[1].toUpperCase()
      if (k === 'AND') { tokens.push({ type: 'AND', value: k }); i += kw[1].length; continue }
      if (k === 'OR')  { tokens.push({ type: 'OR',  value: k }); i += kw[1].length; continue }
      if (k === 'NOT') { tokens.push({ type: 'NOT', value: k }); i += kw[1].length; continue }
      if (k === 'XOR') { tokens.push({ type: 'XOR', value: k }); i += kw[1].length; continue }
    }

    if (ch === '0' || ch === '1') {
      tokens.push({ type: 'CONST', value: ch }); i++; continue
    }
...
class Parser {
  ...
  private parseAtom(): ASTNode {
    ...
    if (tok.type === 'LPAREN') {
      this.consume()
      const node = this.parseOr()
      if (this.peek().type !== 'RPAREN') throw new Error('Очікується ")"')
      this.consume()
      return node
    }
    if (tok.type === 'CONST') {
...
export function extractVariables(node: ASTNode): string[] {
  ...
  function walk(n: ASTNode) {
    ...
    else if (n.type === 'NOT') walk(n.operand)
    else if ('left' in n) { walk(n.left); walk(n.right) }
  }
  walk(node)
...
export function buildTruthTable(expr: string): { table: TruthTableRow[]; variables: string[] } {
  ...
  if (variables.length > 4) throw new Error('Підтримується до 4 змінних')
