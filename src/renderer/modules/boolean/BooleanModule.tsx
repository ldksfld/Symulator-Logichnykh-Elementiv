...
import { expressionToCircuit } from '../logic/utils/astToCircuit'
...
export default function BooleanModule() {
  ...
  const evaluateExpression = useCallback((expr: string, manual: boolean) => {
    if (!expr.trim()) {
      if (manual) {
    ...
    try {
      const { table, variables: vars } = buildTruthTable(expr)
      const dDnf = buildDnf(table, vars)
      const dCnf = buildCnf(table, vars)
      const { simplified, steps } = simplifyExpression(expr)
  ...
  const handleCheckIdentity = useCallback(() => {
    if (!expression.trim() || !expression2.trim()) {
      return
    }
    ...
    try {
      ...
      if (n > 20) {
        setError('Занадто багато змінних для перевірки тотожності')
        return
      }
  ...
  const handleToCircuit = () => {
    ...
    const circuit = expressionToCircuit(expression)

    if (!circuit) {
      setError(uk.boolean.cannotBuildCircuit)
      return
    }

    try {
      sessionStorage.setItem('sleschi-pending-circuit', JSON.stringify(circuit))
    } catch {
    }
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-4">
              ...
              {activeTab === 'karnaugh' && (
                karnaughMap
                  ...
                  : <div className="text-xs text-gray-400 dark:text-gray-500">{uk.boolean.needsVarsForKarnaugh}</div>
              )}
              ...
              {activeTab === 'simplify' && (
                <div className="flex flex-col gap-3">
                  ...
                  {simplificationSteps.length === 0 && (
                    <div className="text-xs text-gray-400 dark:text-gray-500">{uk.boolean.noSimplifications}</div>
                  )}
                </div>
              )}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder={uk.boolean.expr2Placeholder}
                        spellCheck={false}
                      />
                      <button
