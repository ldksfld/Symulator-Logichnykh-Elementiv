...
export function validateInput(value: string, base: Base): string | null {
  if (!value || value === '.' || value === ',') {
    return 'Введіть число для конвертації'
  }
  ...
  if (base === 2) {
    if (!/^[01]*\.?[01]*$/.test(normalized)) {
      ...
      return bad
        ? `Символ "${bad}" не є допустимим для двійкової системи`
        : 'Допустимі символи для двійкової: 0 та 1'
    }
  } else if (base === 16) {
    if (!/^[0-9a-fA-F]*\.?[0-9a-fA-F]*$/.test(normalized)) {
      const bad = normalized.replace(/[.]/g, '').split('').find((c) => !/[0-9a-fA-F]/i.test(c))
      return bad
        ? `Символ "${bad.toUpperCase()}" не є допустимим для шістнадцяткової системи числення`
        : 'Недопустимі символи'
    }
  } else {
    if (!/^\d*\.?\d*$/.test(normalized)) {
      ...
      return bad
        ? `Символ "${bad}" не є допустимим для десяткової системи`
        : 'Допустимі символи для десяткової: цифри 0–9'
    }
  }
...
export function decToBin(value: string, maxFracDigits = 8): ConversionResult {
  ...
  if (!fracStr) {
    return { result: intBin, steps, isInfinite: false }
  }
...
export function decToHex(value: string, maxFracDigits = 8): ConversionResult {
  ...
  if (!fracStr) {
    return { result: intHex, steps, isInfinite: false }
  }
  ...
  const fracHexChars: string[] = []
  let fracStepNum = steps.length + 1

  fracTetrads.forEach((t) => {
    const hexChar = parseInt(t, 2).toString(16).toUpperCase()

    steps.push({
      stepNumber: fracStepNum++,
      operation: `Тетрада ${t} (дробова)`,
      value: hexChar,
      highlight: [t],
    })

    fracHexChars.push(hexChar)
  })

  const fracHex = fracHexChars.join('')
...
export function convert(
  ...
  if (fromBase === toBase) {
    return { result: value.toUpperCase(), steps: [], isInfinite: false }
  }

  if (fromBase === 10 && toBase === 2) {
    return decToBin(value, maxFracDigits)
  }

  if (fromBase === 10 && toBase === 16) {
    return decToHex(value, maxFracDigits)
  }
